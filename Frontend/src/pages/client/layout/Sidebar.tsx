...
const sections: NavSection[] = [
  ...
  },
  {
    label: "FINANCIAL",
    items: [
      { title: "Deposit", icon: Wallet, path: "/client/financial/deposit" },
      { title: "Withdrawal", icon: ArrowDownToLine, path: "/client/financial/withdrawal" },
      { title: "Transfer", icon: ArrowUpDown, path: "/client/financial/transfer" },
      { title: "History", icon: History, path: "/client/financial/history" },
    ],
  },
  {
    label: "TRADING",
    items: [
      { title: "Open Account", icon: PlusCircle, path: "/client/account/new" },
      { title: "Account List", icon: List, path: "/client/account/list" },
      { title: "Contest", icon: Trophy, path: "/client/account/trading-contest" },
      { title: "Platforms", icon: Monitor, path: "/client/trading-platforms" },
    ],
  },
  {
    label: "GROWTH",
    items: [
      { title: "Refer A Friend", icon: Users, path: "/client/refer-friend" },
    ],
  },
  {
    label: "PARTNER ZONE",
    items: [
      { title: "Create IB Account", icon: UserPlus, path: "/client/partner/new-account" },
      { title: "Partner Dashboard", icon: BarChart3, path: "/client/partner/dashboard" },
      { title: "IB Commission", icon: Percent, path: "/client/partner/ib-commission" },
      { title: "IB Withdrawal", icon: CreditCard, path: "/client/partner/ib-withdrawal" },
    ],
  },
]
      ...
      <nav className="flex-1 overflow-y-auto overflow-x-hidden py-3 scrollbar-hidden">
        {sections.map((section) => (
          <div key={section.label} className="mb-1">
            ...
            {shouldShowExpanded && (
              <p
                className="px-4 pt-3 pb-1 text-[10px] font-semibold tracking-widest"
            ...
            {section.items.map((item) => {
              ...
              return (
                ...
                >
                  ...
                  />
                  <span
                    className={`whitespace-nowrap transition-all duration-300 ${shouldShowExpanded ? "opacity-100 w-auto" : "opacity-0 w-0 overflow-hidden"}`}
                  >
                    {item.title}
                  </span>
                  {active && (
      ...
      {shouldShowExpanded && (
        ...
        >
          <div className="flex items-center gap-2">
            ...
            <span className="text-xs font-semibold" style={{ color: "var(--theme-text-disabled)" }}>
              Client Portal
            </span>
          </div>
        </div>
