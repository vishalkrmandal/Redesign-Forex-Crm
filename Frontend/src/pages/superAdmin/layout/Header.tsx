...
import { Menu, Search, LogOut, Shield, ChevronDown, X, Bell, Clock, Palette, Settings, UserPlus } from "lucide-react"
...
const PAGE_TITLES: Record<string, string> = {
  "/superadmin/configure": "Configure",
  "/superadmin/admin-registration": "Admin Registration",
  "/superadmin/theme-settings": "Theme Settings",
}
...
const PAGES: SearchPage[] = [
  { label: "Configure",          path: "/superadmin/configure",          section: "Administration", icon: Settings  },
  { label: "Admin Registration", path: "/superadmin/admin-registration", section: "Administration", icon: UserPlus  },
  { label: "Theme Settings",     path: "/superadmin/theme-settings",     section: "Appearance",     icon: Palette   },
]
...
export default function Header({ toggleSidebar, isMobile }: HeaderProps) {
  ...
  const [userData, setUserData] = useState({ firstname: "Super", lastname: "Admin", email: "superadmin@example.com" })
  ...
  useEffect(() => { if (user) setUserData({ firstname: user.firstname || "Super", lastname: user.lastname || "Admin", email: user.email || "superadmin@example.com" }) }, [user])
  ...
  return (
    <header className="sticky top-0 z-30 flex h-16 items-center justify-between px-4 backdrop-blur-xl" style={{ backgroundColor: "color-mix(in srgb, var(--theme-bg-sidebar) 95%, transparent)", borderBottom: "1px solid var(--theme-border)" }}>
      {/* Left */}
      <div className="flex items-center gap-3">
        <button onClick={toggleSidebar} className="rounded-lg p-2 transition-all duration-200 active:scale-95" style={{ color: "var(--theme-text-muted)" }} onMouseEnter={(e) => { ;(e.currentTarget as HTMLElement).style.backgroundColor = "color-mix(in srgb, var(--theme-primary) 10%, transparent)"; ;(e.currentTarget as HTMLElement).style.color = "var(--theme-primary)" }} onMouseLeave={(e) => { ;(e.currentTarget as HTMLElement).style.backgroundColor = ""; ;(e.currentTarget as HTMLElement).style.color = "var(--theme-text-muted)" }}>
          <Menu className="h-5 w-5" />
        </button>
        {!isMobile && (
          <div className="hidden md:flex items-center gap-2">
            <span className="text-xs" style={{ color: "var(--theme-text-disabled)" }}>Super Admin</span>
      ...
      <div ref={searchContainerRef} className="relative flex-1 max-w-sm mx-4 hidden md:block">
        {showSearch ? (
          ...
          <button onClick={() => setShowSearch(true)} className="flex w-full items-center rounded-lg px-4 py-2 text-sm" style={{ backgroundColor: "var(--theme-bg-card)", border: "1px solid var(--theme-border)", color: "var(--theme-text-disabled)" }} onMouseEnter={(e) => { (e.currentTarget as HTMLElement).style.borderColor = "var(--theme-primary)" }} onMouseLeave={(e) => { (e.currentTarget as HTMLElement).style.borderColor = "var(--theme-border)" }}>
            <Search className="mr-2 h-4 w-4" /><span>Search pages…</span>
          </button>
        )}
      ...
      <div className="flex items-center gap-2">
        ...
        {/* Theme shortcut */}
        <button onClick={() => navigate("/superadmin/theme-settings")} className="rounded-lg p-2" style={{ color: "var(--theme-text-muted)" }} onMouseEnter={(e) => { (e.currentTarget as HTMLElement).style.color = "var(--theme-primary)" }} onMouseLeave={(e) => { (e.currentTarget as HTMLElement).style.color = "var(--theme-text-muted)" }} title="Theme Settings">
        ...
        <div className="relative" ref={userMenuRef}>
            ...
            {!isMobile && (
              <>
                <div className="hidden md:block text-left">
                  ...
                  <p className="text-[10px]" style={{ color: "var(--theme-text-disabled)" }}>Super Admin</p>
                </div>
          ...
          {showUserMenu && (
            <div className="absolute right-0 top-full mt-2 w-64 rounded-xl shadow-2xl overflow-hidden z-50" style={{ backgroundColor: "var(--theme-bg-card)", border: "1px solid var(--theme-border)" }}>
              ...
              <div className="p-2">
                <MenuBtn icon={Settings} label="Configure" onClick={() => { navigate("/superadmin/configure"); setShowUserMenu(false) }} />
                <MenuBtn icon={Palette} label="Theme Settings" onClick={() => { navigate("/superadmin/theme-settings"); setShowUserMenu(false) }} />
