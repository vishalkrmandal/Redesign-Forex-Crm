...
const sections: NavSection[] = [
  ...
  },
  {
    label: "MANAGEMENT",
    items: [
      { title: "Clients", icon: Users, path: "/admin/features/clients" },
  ...
  },
  {
    label: "SETTINGS",
    items: [
      { title: "Configure", icon: Settings, path: "/admin/configure" },
    ],
  },
  {
    label: "PARTNERS",
    items: [
      { title: "IB Partners", icon: HandshakeIcon, path: "/admin/partner/ib-partners" },
      { title: "IB Withdrawals", icon: Banknote, path: "/admin/partner/ib-withdrawals" },
    ],
  },
]
            ...
            {section.items.map((item) => {
              ...
              return (
                ...
                >
                  ...
                  <span className={`whitespace-nowrap transition-all duration-300 ${shouldShowExpanded ? "opacity-100 w-auto" : "opacity-0 w-0 overflow-hidden"}`}>
                    {item.title}
                  </span>
                  {active && (
                    <span className="absolute right-2 h-1.5 w-1.5 rounded-full" style={{ background: "var(--theme-primary)" }} />
                  )}
                </Link>
              )
            })}
      ...
      {shouldShowExpanded && (
        <div className="flex-shrink-0 px-4 py-3" style={{ borderTop: "1px solid var(--theme-border)" }}>
          <div className="flex items-center gap-2">
            ...
            <span className="text-xs font-semibold" style={{ color: "var(--theme-text-disabled)" }}>Admin Panel</span>
          </div>
        </div>
      )}
    </aside>
